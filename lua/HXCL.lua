...
--]]
  if input == "ooo" and string.len(history_oo)>0 then
    yield(Candidate("oo", seg.start, seg._end, history_oo, "get o"))
  elseif input == "uuu" and string.len(history_oo)>0 then
    yield(Candidate("oo", seg.start, seg._end, history_uu, "get u"))
--[[
...
--          file = io.open("C:\\Users\\Yazuu\\AppData\\Roaming\\Rime\\pinyin_simp_pin.txt", "a")
...
---[[
      ...
      local text= string.gsub(text,"&nbsp"," ")
      local text= string.gsub(text,">br<","<br>")
      local comment= string.gsub(cand.comment,"<br>",next_line)
      local comment= string.gsub(comment,"&nbsp"," ")
--]]
--      local comment = cand.comment

      
      if string.find(text, "")~=nil then
        text = string.gsub(text,"",history_oo)
        if string.len(history_uu)>0 then
          text = string.gsub(text,"",history_uu)
        end
        comment =  "=" .. history_oo
      end
      
---[[
      if string.len(text)>120 and context:get_option('HXZD') then
...
--      if string.len(text)>110 then
      if string.len(text)>110 and context:get_option('HXZD') then
        local key = get_sub_string(text,100)
--        local key = string.sub(text,0,100)
        oo_buffer[key] = text
...
--  local file = io.open("C:\\Users\\Yazuu\\AppData\\Roaming\\Rime\\history.txt","a")
--  file:write("\n" .. commit_text .. "\tk=" .. k .. " v=" .. v)
--  file:close()
